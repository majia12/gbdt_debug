...
from decision_tree.tree_core.criterion import XgboostCriterion
...
class Splitter(object):
    ...
    def find_split_single_histogram_guest(self, histogram, valid_features, use_missing, zero_as_missing):
        # default values
        best_fid = None
        ...
        for fid in range(len(histogram)):
            ...
            if node_cnt < self.min_sample_split:
                # print('ERROR:splitter.py {}'.format(node_cnt))
                break
        ...
        print((best_fid, best_bid, best_gain))
        splitinfo = SplitInfo(best_fid=best_fid, best_bid=best_bid,
                              gain=best_gain, sum_grad=best_sum_grad_l, sum_hess=best_sum_hess_l)

        return splitinfo

    def find_split(self, histograms, valid_features, partitions=1, use_missing=False, zero_as_missing=False):

        splitinfo_table = [self.find_split_single_histogram_guest(histogram, valid_features, use_missing, zero_as_missing)
